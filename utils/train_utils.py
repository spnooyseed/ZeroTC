...
def get_train_data_with_pseudo_label(
    encoder: ZeroTC, train_data: DataLoader, label_names: list
):
    encoder.eval()
    ...
    for data, _ in tqdm(train_data):
        ...
        dataset.append(data)
    logging.info(
        f"--- get_train_data_with_pseudo_label --- accuracy = {get_accuracy(predict , labels)}"
    )
...
def train(encoder: ZeroTC, train_data: DataLoader, label_names: list):
    train_data_with_pseudo_label = get_train_data_with_pseudo_label(
        encoder, train_data, label_names
    )
    import pdb
